...
class Game(arcade.Window):
    ...
    def __init__(self, width, height):
        ...
        # Create a list for targets (similar to the above bullets)
        self.targets = []
    ...
    def update(self, delta_time):
        ...
        if self.bomb_mode:
            self.timer -= 1
            for target in self.targets:
                if isinstance(target, SafeTarget):
        ...
        # Iterate through the targets and tell them to advance
        for target in self.targets:
            target.advance()
    ...
    def create_target(self):
        ...
        self.targets.append(random.choice([StandardTarget(), StrongTarget(), SafeTarget()]))

        if random.randint(1, 20) == 1:
            self.bombs.append(Bomb())
            self.targets.append(Bomb())
